...
async function init() {
  ...
  folder1.open();
  folder1.add(pLight.position, "x", -500, 500, 10);
  folder1.add(pLight.position, "y", -500, 500, 10);
  folder1.add(pLight.position, "z", -500, 500, 10);

  const folder2 = gui.addFolder("Color");
  folder2.open();
  folder2.add(COLORS, "MAIN").onChange(() => {
    mainMate.color.set(COLORS.MAIN);
  });
  folder2.add(COLORS, "SUB").onChange(() => {
